...
#   ci_ibmcloud_save_artifact as the asset-key for load_artifact integration.
# Input:
#   $1 - sbom_file     : Path to the SBOM file on disk
#   $2 - image_ref     : Fully-qualified image reference the SBOM describes
#   $3 - artifact_name : (optional) Artifact name from ci_ibmcloud_save_artifact
#   $4 - architecture  : (optional) Architecture for multi-platform builds
# Output:
...
#   Enhanced to support per-architecture evidence for multi-platform builds.
# =============================================================================
ci_collect_sbom_evidence() {
    local sbom_file="${1:-}"
    local image_ref="${2:-}"
    ...
    # Append architecture to artifact name if specified (for multi-platform builds)
    if [[ -n "$arch" ]]; then
        artifact_name="${artifact_name}-${arch}"
        log_info "Collecting SBOM evidence: asset-key=$artifact_name arch=$arch image=${image_ref:-<unspecified>}"
    else
