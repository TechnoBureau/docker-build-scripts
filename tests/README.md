...
| `hummingbird/test_rootfs_transactions.py` | Private transaction mount namespaces, unchanged seed content, RPM failures/signals, isolation failure and caller-side mount checks. Real native probes include denied proc/sys unmounts and mount/chroot lifetime checks (the test harness creates an isolated user/mount namespace when available) |
...
Native probes test actual `/proc/self/exe` visibility in a chroot, caller-side
mount isolation on success/failure/signals, and operation with proc/sys unmounts
denied. They also verify that the transaction preserves its user/PID namespaces.
The harness is skipped where Linux user/mount namespaces are unavailable; it is
**not** a Rosetta, QEMU or RPM transaction test.
