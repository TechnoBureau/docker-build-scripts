...
## Key design decision: render-time → runtime variant switching

Today `is_builder_variant()` switches package cleanup, dnf config, and
`CONTAINER_DEFAULT_USER` at **render** time, which forces one rendered
Containerfile per variant. To get one Containerfile for all variants, the four
macros below change to **runtime** switches driven by build args, with defaults
rendered from context. The per-variant renders in the containers repo keep
identical behavior because each rendered file bakes its own defaults.

Upstream macro changes (containers repo, then re-sync):

1. `setup_newroot.yml.j2`: `ARG VARIANT={{ variant }}` (baked default);
   builder-only dnf config becomes
   `RUN if [ "${VARIANT}" = "builder" ]; then mkdir -p ... && echo ...; fi`.
2. `cleanup_newroot.yml.j2`: package removal + license/locale cleanup becomes
   `RUN if [ "${VARIANT}" != "builder" ]; then rpm --root ${NEWROOT} -e ... --nodeps && rm -rf ...; fi`.
3. `verify_compliance.yml.j2`: paths become `ARG VARIANT_PATH={{ distro }}/{{ variant }}`
   and `/run/src/${VARIANT_PATH}` at runtime.
4. `final_stage.yml.j2`: `ARG ARCHIVE_PATH=out-${TARGETARCH}.ociarchive` (also
   referenced in the chunkah redirect); `FROM oci-archive:${ARCHIVE_PATH}`;
   `CONTAINER_DEFAULT_USER` unconditional.

Validation: `make all` regenerates per-variant Containerfiles with identical
semantics (compare pre/post diffs); rebuild both curl variants.
...
2. Pure copy (no patches): the ARG-based macro adaptation happens upstream so
   the vendored tree never diverges.
...
- `ci_hummingbird_generate <dir>`: renders into `<dir>/.hbgen/` (gitignored):
...
- `ci_hummingbird_build <dir>`: for each variant from `properties.yml`
  ...
  2. set `CONFIG[CUSTOM_TAGS]`, variant build args (VARIANT, VARIANT_PATH,
     MAIN_PACKAGES default; `ARCHIVE_PATH` left to ci-build.sh hook)
  3. call `ci_build_and_push` with the generated Containerfile + context `<dir>`
...
2. `ci_build_and_push` (ci-build.sh): add a chunkah hook driven by
   ...
   - per-arch archive collision handling: in the multi-platform parallel loop,
     pass `--build-arg ARCHIVE_PATH=out-${TARGETARCH}.ociarchive` (verified
     against buildah behavior; fallback documented in Risks).
...
1. `FROM oci-archive:${ARCHIVE_PATH}` with buildah: ARG interpolation in FROM
   with per-arch values must be verified. Fallback: driver renders per-arch
   FROM lines (single j2, per-arch FROM substitution at driver level).
2. Multi-platform parallel builds share the `/run/src` bind mount: unique
   per-arch archive filenames assume the FROM resolves per platform. Fallback:
   sequential per-platform builds for chunkah images
...
1. containers: macro adaptation (ARG-based variants) + validation
2. docker-build-scripts: `tools/sync-hummingbird.sh` + vendored `hummingbird/`
3. docker-build-scripts: `ci-core.sh` custom strategy + `ci-hummingbird.sh`
4. docker-build-scripts: `ci-build.sh` chunkah hook + driver dispatch
5. docker-build-scripts: `ci-vuln.sh` + SBOM re-enable
6. docker-builds: `builders/curl` hummingbird definition
7. docker-builds: test results / fixes
